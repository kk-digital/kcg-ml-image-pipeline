...
from kandinsky_worker.dataloaders.prompt_embedding import PromptEmbedding
...
def get_embeddings(job_uuid,
    ...
    positive_embedding, _, positive_mask = text_embedder.compute_embeddings(positive_prompt)
    negative_prior_embedding, _, negative_prior_mask = text_embedder.compute_embeddings(negative_prior_prompt)
    negative_decoder_embedding, _, negative_decoder_mask = text_embedder.compute_embeddings(negative_decoder_prompt)

    positive_mask_detached = positive_mask.detach().cpu().numpy()
    negative_prior_mask_detached = negative_prior_mask.detach().cpu().numpy()
    negative_decoder_mask_detached = negative_decoder_mask.detach().cpu().numpy()
    prompt_embedding = PromptEmbedding(job_uuid,
                                       ...
                                       negative_prior_embedding.detach().cpu().numpy(),
                                       negative_decoder_embedding.detach().cpu().numpy(),
                                       positive_mask_detached,
                                       negative_prior_mask_detached,
                                       negative_decoder_mask_detached)

    # average
    positive_average_pooled = tensor_attention_pooling(positive_embedding, positive_mask)
    negative_prior_average_pooled = tensor_attention_pooling(negative_prior_embedding, negative_prior_mask)
    negative_decoder_average_pooled = tensor_attention_pooling(negative_decoder_embedding, negative_decoder_mask)
    ...
    prompt_embedding_average_pooled = PromptEmbedding(job_uuid,
                                       ...
                                       negative_prior_average_pooled.detach().cpu().numpy(),
                                       negative_decoder_average_pooled.detach().cpu().numpy(),
                                       positive_mask_detached,
                                       negative_prior_mask_detached,
                                       negative_decoder_mask_detached)
    ...
    negative_prior_max_pooled = tensor_max_pooling(negative_prior_embedding)
    negative_decoder_max_pooled = tensor_max_pooling(negative_decoder_embedding)
    prompt_embedding_max_pooled = PromptEmbedding(job_uuid,
                                                  ...
                                                  negative_prior_max_pooled.detach().cpu().numpy(),
                                                  negative_decoder_max_pooled.detach().cpu().numpy(),
                                                  positive_mask_detached,
                                                  negative_prior_mask_detached,
                                                  negative_decoder_mask_detached)
    ...
    negative_prior_signed_max_pooled = tensor_max_abs_pooling(negative_prior_embedding)
    negative_decoder_signed_max_pooled = tensor_max_abs_pooling(negative_decoder_embedding)
    prompt_embedding_signed_max_pooled = PromptEmbedding(job_uuid,
                                                          ...
                                                          negative_prior_signed_max_pooled.detach().cpu().numpy(),
                                                          negative_decoder_signed_max_pooled.detach().cpu().numpy(),
                                                          positive_mask_detached,
                                                          negative_prior_mask_detached,
                                                          negative_decoder_mask_detached)
