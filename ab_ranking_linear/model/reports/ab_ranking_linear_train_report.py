def get_train_report(model_class, dataset_path, training_percent, base_training_size, training_size, base_validation_size, validation_size, train_sum_correct,
    ...
    report_str += "base-training-image-count: {0} ({1:04}%)\n".format(base_training_size,
                                                                 round(training_percent * 100, 2))
    report_str += "base-validation-image-count: {0} ({1:04}%)\n".format(base_validation_size,
                                                                   round((1 - training_percent) * 100, 2))

    report_str += "training-image-count-after-split-shuffle: {0} ({1:04}%)\n".format(training_size,
                                                                 round(training_percent * 100, 2))
    report_str += "validation-image-count-after-split-shuffle: {0} ({1:04}%)\n".format(validation_size,
                                                                   round((1 - training_percent) * 100, 2))
    ...
    report_str += "Training image count after split shuffle: {0}\n".format(training_size)
    ...
    report_str += "Validation image count after split shuffle: {0}\n".format(validation_size)
