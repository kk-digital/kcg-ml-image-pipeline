...
class ScoringTreeConnectNetwork(nn.Module):
    def __init__(self, minio_client, input_size=1280, input_type="input_clip" , output_size=1, 
        ...
        self.model = self.model = TreeConnectArchitectureTanhRanking(input_size).to(self._device)
    ...
    def get_model_path(self):
        local_path=f"output/{self.output_type}_treeconnect_{self.input_type}.pth"
        minio_path=f"{self.dataset}/models/latent-generator/{self.date}_{self.output_type}_treeconnect_{self.input_type}.pth"
    ...
    def load_model(self):
        ...
        suffix= f"_{self.output_type}_treeconnect_{self.input_type}.pth"
