...
def get_graph_report(training_loss,
    ...
    phrase_scores_histogram = plt.subplot2grid(figure_shape, (7, 0), rowspan=1, colspan=2)
    ...
    if x_chronological_sigma_scores is not None and y_chronological_sigma_scores is not None:
        ...
        extent = [x[0] - (x[1] - x[0]) / 2., x[-1] + (x[1] - x[0]) / 2., 0, 1]
        c = chronological_pred_sigma_scores_target_1_plot.imshow(y[np.newaxis, :], cmap="plasma", aspect="auto",
                                                                 extent=extent)
