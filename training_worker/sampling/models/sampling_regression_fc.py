...
class SamplingFCRegressionNetwork(nn.Module):
    ...
    def train(self, n_spheres, target_avg_points, learning_rate=0.001, validation_split=0.2, num_epochs=100, batch_size=256):
        ...
        if self.input_type == "uniform_sphere":
            ...
            inputs, outputs = self.dataloader.load_sphere_dataset(n_spheres=n_spheres,target_avg_points=target_avg_points, percentile=self.sampling_parameter["percentile"], std=self.sampling_parameter["std"])
        else:
            return None
