...
class RapidlyExploringTreeSearch:
    ...
    def label_nodes_by_topic(self, tree):
        ...
        for start_index in range(0, num_nodes, self.batch_size):
            ...
            nodes = torch.stack(tree[start_index:end_index]).to(device=self.device)
    ...
    def graph_datapoints(self, tree, labels, generation_size):
        ...
        min_x, min_y= np.min(umap_embeddings[:,0], axis=1) , np.min(umap_embeddings[:,1], axis=1)
        max_x, max_y= np.max(umap_embeddings[:,0], axis=1) , np.max(umap_embeddings[:,1], axis=1)
