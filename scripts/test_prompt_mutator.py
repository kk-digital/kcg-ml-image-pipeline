...
def get_best_substitution_choice(xgboost_model ,prompt_str, 
    ...
    for token in range(token_number):
        ...
        # add delta score to the list of scores
        delta_score=xgboost_model.predict([substitution_input])[0]
        if delta_score>0:
            delta_scores.append(-delta_score)
    
    tokens_to_substitute=np.argsort(delta_scores)

    return tokens_to_substitute

def mutate_prompt(device, embedding_model,
                  xgboost_model, scoring_model, 
                  prompt_str, phrase_list, 
                  max_iterations=1000, early_stopping=10):
    ...
    for i in range(max_iterations):
        print(f"iteration {i}")
        substitution_tokens=get_best_substitution_choice(xgboost_model, 
                                                        prompt_str,
                                                        prompt_score,
                                                        prompt_embedding, 
                                                        phrase_embeddings)
        ...
        for token in substitution_tokens:
            ...
            phrase_embeddings[token]= get_prompt_embedding(device, embedding_model, prompt_list[token])
...
def main():
    ...
    prompt_mutator= PromptMutator(minio_client=minio_client, output_type="delta_score")
    prompt_mutator.load_model()
