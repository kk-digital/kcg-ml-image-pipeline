...
#     def __init__(self, dataset, class_idx=3):  # Set class_idx=3 for cats
#         self.dataset = dataset
#         self.class_idx = class_idx

#     def __len__(self):
#         cat_indices = [i for i, label in enumerate(self.dataset.targets) if label == self.class_idx]
#         return len(cat_indices)

#     def __getitem__(self, idx):
#         cat_indices = [i for i, label in enumerate(self.dataset.targets) if label == self.class_idx]
#         img, target = self.dataset[cat_indices[idx]]
#         return img, target

# class DogsDataset(torch.utils.data.Dataset):
#     def __init__(self, dataset, class_idx=5):  # Set class_idx=5 for dogs
#         self.dataset = dataset
#         self.class_idx = class_idx

#     def __len__(self):
#         dog_indices = [i for i, label in enumerate(self.dataset.targets) if label == self.class_idx]
#         return len(dog_indices)

#     def __getitem__(self, idx):
#         dog_indices = [i for i, label in enumerate(self.dataset.targets) if label == self.class_idx]
#         img, target = self.dataset[dog_indices[idx]]
#         return img, target
...
cat_idx = np.where((np.array(train_set.targets) == 2))[0]
...
# cats_dataset_v = val_set[cat_index_t]
# dogs_dataset_v = val_set[filter[5], ...]
